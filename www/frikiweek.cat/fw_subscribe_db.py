...
def llista_tallers(db):
	...
	l = []
	cursor = getCursor(db)
	cursor.execute("SELECT t.*, COUNT(i.*) AS recompte FROM taller t LEFT JOIN inscripcio i ON t.id = i.id_taller WHERE Year(t.data) = Year(%s) GROUP BY t.id ORDER BY t.data", (datetime.datetime.now().strftime('%Y-%m-%d'),))
